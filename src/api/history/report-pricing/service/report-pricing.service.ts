...
export class ReportPricingService {
  constructor(
    @InjectModel(ReportPricingHistory.name) private reportPricingHistory: Model<ReportPricingHistory>,
  ) {}

  async create(createReportPricingInterface: CreateReportPricingInterface): Promise<ReportPricingHistory> {
    const createdReportPricing = new this.reportPricingHistory(createReportPricingInterface);
