...
export class AddressRepository extends Repository<AddressEntity> implements RepositoriesAbstract<AddressEntity,CreateAddressDto,UpdateAddressDto>{
constructor(@Inject(PostgresConstant) private postgresDataSource:DataSource){
    ...
    async findOneEntity(id: string): Promise<AddressEntity> {
        return await this.createQueryBuilder("address").where("address.id=:address_id",{address_id:id}).getOne()
    }
