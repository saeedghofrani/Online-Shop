...
@Injectable()
export class CityRepository extends Repository<CityEntity> implements RepositoriesAbstract<CityEntity,CreateCityDto,UpdateCityDto>{
   ...
   async updateEntity(id: string, updateEntityDto: UpdateCityDto): Promise<UpdateResult> {
        return await this.update(id,updateEntityDto)
    }
   async findOneEntity(id: string): Promise<CityEntity> {
        return await this.createQueryBuilder("city").where("city.id=:city_id",{city_id:id}).getOne()
    }
