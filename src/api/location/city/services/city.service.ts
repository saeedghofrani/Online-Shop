...
export class CityService{
    ...
    async createEntity(createEntityDto: CreateCityDto): Promise<CityEntity> {
        const findState=await this.stateService.findOneEntity(createEntityDto.stateId)
   ...
   async updateEntity(id: string, updateEntityDto: UpdateCityDto): Promise<UpdateResult> {
    const findState=await this.stateService.findOneEntity(updateEntityDto.stateId)
