...
@Injectable()
export class StateRepository extends Repository<StateEntity> implements RepositoriesAbstract<StateEntity,CreateStateDto,UpdateStateDto>{
   ...
   async updateEntity(id: string, updateEntityDto: UpdateStateDto): Promise<UpdateResult> {
        return await this.update(id,updateEntityDto)
    }
   async findOneEntity(id: string): Promise<StateEntity> {
        return await this.createQueryBuilder("state").where("state.id=:state_id",{state_id:id}).getOne()
    }
