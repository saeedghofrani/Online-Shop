...
import { DataSource, Repository, RepositoryNotTreeError, UpdateResult } from "typeorm";
import { CreatePermissionDto } from "../dto/create.permission.dto";
import { UpdatePermissionDto } from "../dto/update.permission.dto";

@Injectable()
export class PermissionRepository extends Repository<PermissionEntity> implements RepositoriesAbstract<PermissionEntity,CreatePermissionDto,UpdatePermissionDto>{
    constructor(@Inject() private postgresDataSource:DataSource)
    {
        super(PermissionEntity,postgresDataSource.createEntityManager())
    }
   async createEntity(createEntityDto: CreatePermissionDto): Promise<PermissionEntity> {
        return await this.save(this.create(createEntityDto))
    }

   async updateEntity(id: string, updateEntityDto: UpdatePermissionDto): Promise<UpdateResult> {
