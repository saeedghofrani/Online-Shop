...
@Injectable()
export class AttributeRepository extends Repository<AttributeEntity> implements RepositoriesAbstract<AttributeEntity,CreateAttributeDto,UpdateAttributeDto>{
   ...
   async findOneEntity(id: string): Promise<AttributeEntity> {
        return await this.createQueryBuilder("attribute").where("attribute.id=:attribute_id",{attribute_id:id}).getOne()
    }
