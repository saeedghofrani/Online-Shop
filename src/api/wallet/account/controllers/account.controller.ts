...
export class AccountController{
    ...
    @Post()
    createEntity(@Body() createEntityDto: CreateAccountDto,@GetUser() user:UserInterface): Promise<AccountEntity> {
        return this.accountService.createEntity(createEntityDto,user.userId)
    }
    @Patch()
    updateEntity(@Query("account_id") id: string, @Body() updateEntityDto: UpdateAccountDto): Promise<UpdateResult> {
