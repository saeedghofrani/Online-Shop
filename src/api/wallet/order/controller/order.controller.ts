...
export class OrderController{
    ...
    @Post()
    async createEntity(@Body() createEntityDto: CreateOrderDto,@Query("user_Id") userId:string,@Query("product_id") pricingId:string): Promise<OrderEntity> {
    ...
    @Patch()
    async updateEntity(@Query("order_Id") id: string,@Body() updateEntityDto: UpdateOrderDto,@Query("user_Id") userId:string,@Query("product_id") pricingId:string): Promise<UpdateResult> {
        return this.orderService.updateEntity(id, updateEntityDto, userId, pricingId)
    }
