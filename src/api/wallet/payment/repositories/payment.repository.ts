...
@Injectable()
export class PaymentRepository extends Repository<PaymentEntity> implements RepositoriesAbstract<PaymentEntity, CreatePaymentDto,UpdatePaymentDto>{
    ...
    async createEntity(createEntityDto: CreatePaymentDto): Promise<PaymentEntity> {
    ...
    async findOneEntity(id: string): Promise<PaymentEntity> {
        return await this.createQueryBuilder("payment").where("payment.id=:payment_id",{payment_id:id}).getOne()
    }
