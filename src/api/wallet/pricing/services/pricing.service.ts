...
export class PricingService {
    ...
    async createEntity(createEntityDto: CreatePricingDto,productId:string): Promise<PricingEntity> {
        try {
    ...
    async updateEntity(id: string, updateEntityDto: UpdatePricingDto,productId:string): Promise<UpdateResult> {
        try {
            const findProduct=await this.productService.findOneEntity(productId)
            if(findProduct)
                updateEntityDto.product=findProduct
