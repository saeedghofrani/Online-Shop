...
export class UserFactorService{
    constructor(private userFactorRepository:UserFactorRepositories,
    private patternMasterService:PatternMasterService) {
    }
    async createEntity(createEntityDto: CreateUserFactorDto,patterMasterId:string): Promise<UserFactorEntity> {
        const findPatterMaster=await this.patternMasterService.findOneEntity(patterMasterId)
        if(!findPatterMaster)
            throw new Error()
