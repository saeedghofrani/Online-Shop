...
@Injectable()
export class WalletRepository extends Repository<WalletEntity> implements RepositoriesAbstract<WalletEntity, CreateWalletDto,UpdateWalletDto>{
    ...
    async findOneEntity(id: string): Promise<WalletEntity> {
        return await this.createQueryBuilder("wallet").where("wallet.id=:id_wallet",{id_wallet:id})
            .getOne()
    }
